...
import {Flex,Circle,Box,Image,useColorModeValue, Badge,Icon,Button,Tooltip,Stack,Link,HStack,Text, useToast} from '@chakra-ui/react'
...
const addItem =(id) =>{
  if(cart.some((cartItem)=>cartItem.id===id)){
     toast({
      description:'This itme is already exist, Go to your cart to change the amount',
      status:'error',
      isClosable:true,
     });
  }else{
    ...
    toast({
      description:'Item has been added', status:'success',isClosable:true
    });
  }
};
    ...
    <Stack p='2' spacing='3px' bg={useColorModeValue('white','gray.800')} minW='240px' h='450px' borderWith='1px' rounded='lg' shadow='lg' position='relative'>
       ...
       <Flex mt='1' justifyContent='space-between' alignContent='center'>
            <Link as ={ReactLink} to = {`/product${product._id}`} pt='2' cursor='pointer'>
              <Box fontSize='1.8xl' fontWeight='semibold' lineHeight='tight'>
              {product.name}
              </Box>
            </Link>
       </Flex>
       ...
       <Flex justify='space-between' >
          ...
          <Button variant='ghost' display={'flex'} isDisabled={product.stock <= 0} onClick={() => addItem(product.id)}>
